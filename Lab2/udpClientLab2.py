...
if len(sys.argv) < 5:
	...
	"followed by a requestID number (0-127), then one more more strings of "
    "websites like www.google.com !")
	sys.exit()
...
"""
	TODO: Should eventually check if the magic number (send and recieve) works
	on other teams' clients and servers.
"""
...
"""
	TODO: Calculate and program the checksum. It is calculated by adding the
	binary of each byte. In the event of a carry in the addition, add the
	carry to the Least Significant Bit in the result and continue to add the
	next number. At the end, take the number we get and do 1's compliment on
	it and that number will be our checksum.
	(note: the checksum we get should give us 1111 1111 when added to the
	result of all the other bytes combined.)
"""
...
"""
	TODO: Figure out if IPs coming back are correct. Likely not. So how to read
	or send or recieve them correctly...???
"""
